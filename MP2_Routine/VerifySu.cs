...
namespace MP2
{
    public class VerifySuApiClient
    {
        ...
        {
            string userKey = Mp2RoutineSettings.Instance.UserKey;  // A chave precisa ser capturada da UI ou de um arquivo
            string hwid = GetHardwareId();
            bool isValid = await VerifySu.VerifyKey(userKey, hwid);
            if (!isValid)
            {
                Mp2Routine.Log.Error("[Mp2] Acess Denied!.");
                BotManager.Stop();
                return false;
            }
            return true;
        ...
        public async Task<bool> VerifyKey(string key, string hwid)
        {
            try
            {
                ...
                {
                    Mp2RoutineSettings.Instance.DaysLeft = "Access denied.";
                    Mp2Routine.Log.Error($"[Mp2] Acess Denied");
                    return false;
                }
            ...
            {
                Mp2Routine.Log.Error($"[Verify.SU] Key error");
                return false;
            }
        }
        ...
        private async Task<bool> ValidateResponse(string response, string expectedHexTick)
        {
            ...
            {
                var DaysLeft = await GetKeyExpirationDate(Mp2RoutineSettings.Instance.UserKey);
                if (DaysLeft == null) DaysLeft = "1";
                Mp2RoutineSettings.Instance.DaysLeft = DaysLeft;
                Mp2Routine.Log.Info("[Verify.SU] Access sucessuful.");
                return true;
            }
            ...
            {
                Mp2Routine.Log.Info("[Verify.SU] Please update your Mp2 Bot.");
                return false;
            }
            if (decryptedCode == "0A14")
            {
                Mp2RoutineSettings.Instance.DaysLeft = "Your key is expired.";
                Mp2Routine.Log.Info("[Verify.SU] Your Key is expired.");
                return false;
            }
            if (decryptedCode == "3FC9")
            {
                Mp2RoutineSettings.Instance.DaysLeft = "Cant find your key.";
                Mp2Routine.Log.Info("[Verify.SU] Cant find your key.");
                return false;
            }
            if (decryptedCode == "903F")
            {
                Mp2RoutineSettings.Instance.DaysLeft = "Key from another product.";
                Mp2Routine.Log.Info("[Verify.SU] Key from another product.");
                return false;
            }
            ...
            {
                Mp2RoutineSettings.Instance.DaysLeft = "Access denied.";
                Mp2Routine.Log.Info("[Verify.SU] Private access denied.");
                return false;
            }

            if (decryptedCode == "3AEE")
            {
                Mp2RoutineSettings.Instance.DaysLeft = "HWID banned.";
                Mp2Routine.Log.Info("[Verify.SU] HWID banned");
                return false;
            }
        ...
        public async Task<string?> GetKeyExpirationDate(string userKey)
        {
            try
            {
                ...
                Mp2Routine.Log.Warn("[Verify.SU] Error: ");
                return null;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> SendMessageToVerifySu(string message, string color = "")
        {
            try
            {
                ...
                string requestUrl = $"{ApiUrl}sendmsg/?api={_apiKey}&id={ProductId}&hwid={GetHardwareId}&key={Mp2RoutineSettings.Instance.UserKey}&msg={encodedMessage}";
