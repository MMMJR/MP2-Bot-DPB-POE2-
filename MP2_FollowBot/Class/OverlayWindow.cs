...
namespace MP2.Class
{
	public class OverlayWindow : IDisposable
    {
		...
		private void _window_SetupGraphics(object sender, SetupGraphicsEventArgs e)
		{
            ...
            _brushes["transparent_black"] = gfx.CreateSolidBrush(0, 0, 0, Mp2Settings.Instance.OverlayTransparency);
		...
		private void _window_DrawGraphics(object sender, DrawGraphicsEventArgs e)
		{
            var gfx = e.Graphics;
			gfx.ClearScene();
			if (!Mp2Settings.Instance.EnableOverlay)
			{
				return;
			}

			if (!Mp2Settings.Instance.DrawInBackground && LokiPoe.ClientWindowHandle != GetForegroundWindow())
			{
				return;
			}
            ...
            //gfx.DrawTextWithBackground(_fonts["consolas"], _brushes["green"], _brushes["transparent_black"], Mp2Settings.Instance.OverlayXCoord, Mp2Settings.Instance.OverlayYCoord, botInfoText);
			if (!LokiPoe.IsInGame) return;

            if (Mp2Settings.Instance.DrawMobs || Mp2Settings.Instance.DrawCorpses)
				DrawMobs(gfx);
        }
			...
			foreach (var monst in monsts)
			{
				foreach (var networkObject in monst.Value)
				{
					try
					{
						...
						if (monter.IsActiveDead && !Mp2Settings.Instance.DrawCorpses) continue;
						if (!monter.IsActiveDead && !Mp2Settings.Instance.DrawMobs) continue;
        ...
        public void SetTransparency(int overlayTransparency)
        {
			...
			_brushes["transparent_black"] = _window.Graphics.CreateSolidBrush(0, 0, 0, Mp2Settings.Instance.OverlayTransparency);
		}
    }
}
