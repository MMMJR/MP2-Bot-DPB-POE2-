...
namespace MP2.EXtensions
{
    public static class PlayerAction
    {
        ...
        public static async Task<bool> Interact(NetworkObject obj, Func<bool> success, string desc, int timeout = 3000)
        {
            ...
            if (name == "Map Device") return true;
            return false;
        }
        ...
        {
            if (ErrorManager.GetErrorCount("TpToTown") > 8)
            {
            ...
            /*if (!area.IsOverworldArea && !area.IsMap && !area.IsCorruptedArea && !area.IsMapRoom && !area.IsTempleOfAtzoatl && area.Name != "Syndicate Hideout" && !area.IsAtlasMap)
            {
        ...
        public static async Task<bool> TakeTransition(AreaTransition transition, bool newInstance = false)
        {
            ...
            if (((int)type == (int)TransitionType.Local) || ((int)type == (int)TransitionType.Regular))
            {
        ...
        public static Portal PortalInRangeOf(int range)
        {
        ...
        {
            if (LokiPoe.InGameState.CanOpenTownPortal)
            {
                LokiPoe.InGameState.OpenPortal();
            }
            Portal portal = null;
            await Wait.For(() => (portal = PortalInRangeOf(40)) != null, "portal spawning", 100, 6000);
