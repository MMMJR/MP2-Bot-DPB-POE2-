...
namespace MP2.EXtensions
{
    public static class ClassExtensions
    {
        ...
        [CanBeNull]
        public static T ClosestSimulacrumPosition<T>(this IEnumerable collection) where T : SimulacrumArea
        {
            T closest = null;
            foreach (var element in collection)
            {
                ...
                if (typed != null)
                {
                    ...
                    {
                        GlobalLog.Info("Debug Area Position: " + typed.Position.Distance);
                        // && (typed.Position.DistanceSqr >= 10 && typed.Position.DistanceSqr <= 100)
                        if ((closest == null || typed.Position.Distance <= closest.Position.Distance))
                        {
                            closest = typed;
                        }
                    }
        ...
        public static T ClosestValid<T>(this IEnumerable<T> collection, Func<T, bool> match) where T : CachedObject
        {
            ...
            foreach (var element in collection)
            {
                if (!element.Ignored && !element.Unwalkable && match(element))
                {
                    if(element.IsMonster && element.Object != null)
                    {
                        if (!TrackMobLogic.ClosedDoorBetween(LokiPoe.Me, element.Object, 30, 20, false))
                            continue;
                    }
                    if (closest == null || element.Position.DistanceSqr < closest.Position.DistanceSqr)
                        closest = element;
