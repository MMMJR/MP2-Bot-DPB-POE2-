...
namespace MP2.Helpers
{
    public static class PartyHelper
    {
        ...
        public static async Task<bool> HandlePartyInvite()
        {
            ...
            {
                //MP2.Log.WarnFormat($"[FollowBot] Visible Notifications: {LokiPoe.InGameState.NotificationHud.NotificationList.Where(x => x.IsVisible).ToList().Count}");
                LokiPoe.InGameState.ProcessNotificationEx isPartyRequestToBeAccepted = (x, y) =>
                {
                    ...
                    var res = true;
                    //MP2.Log.WarnFormat($"[FollowBot] Detected {y.ToString()} request from char: {x.CharacterName} [AccountName: {x.AccountName}] Accepting? {res}");
                    return res;
                };
                ...
                //MP2.Log.WarnFormat($"[HandlePartyInvite] Result: {ret}");
