...
namespace MP2.EXtensions.Tasks
{
    public class OpenChestTask : ITask
    {
        ...
        private static readonly Mp2Settings Settings = Mp2Settings.Instance;
        ...
        public async Task<bool> Run()
        {
            ...
            GlobalLog.Debug("OpenChest");
            if (Settings.ChestOpenRange != 0)
            {
                if (_chest != null)
                {
                    GlobalLog.Debug("OpenChest2");
                    await ProcessChest();
                    return true;
                }
        ...
        private static async Task ProcessChest()
        {
            ...
            if (pos.IsFar)
            {
                GlobalLog.Debug("[OpenChestTasks] Abandoning current chest because its too far away.");
                GlobalLog.Debug("chestAttempts :" + chestAtt);
                if (chestAtt >= 7)
                {
            ...
            GlobalLog.Debug("[OpenChestTasks]");
            var attempts = ++_chest.InteractionAttempts;
            GlobalLog.Debug("chestAttempts3 :" + _chest.InteractionAttempts);
        ...
        private static bool ShouldOpen(CachedObject obj, int openRange, List<Mp2Settings.ChestEntry> settingsList)
        {
            if (openRange != -1)
            {
